import "./App.css";
import { candidates, TCandidate } from "./schemas/candidate.type";
import { CardCandidato } from "./component/cardCandidate";

function App() {
  const listaCandidates: Array<TCandidate> = candidates;

  return (
    <>
      <h1 className="text-center text-3xl">Bipartisan Vote</h1>
      {listaCandidates.map((c) => (
        <CardCandidato key={c.name} candidatin={c}></CardCandidato>
      ))}
    </>
  );
}
